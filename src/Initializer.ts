import GeographyService from "./geography/GeographyService";
import Street from "./model/Street";
import PlayerService from "./players/PlayerService";
import PropertyService from "./property/PropertyService";

const NEIGHBOURHOOD_NAMES = [
  'Arbutus Ridge',
  'Downtown',
  'Dunbar-Southlands',
  'Fairview',
  'Grandview-Woodland',
  'Hastings-Sunrise',
  'Kensington-Cedar Cottage',
  'Kerrisdale',
  'Killarney',
  'Kitsilano',
  'Marpole',
  'Mount Pleasant',
  'Oakridge',
  'Renfrew-Collingwood',
  'Riley Park',
  'Shaugnessy',
  'South Cambie',
  'Strathcona',
  'Sunset',
  'Victoria-Fraserview',
  'West End',
  'West Point Grey',
];

const STREET_NAMES = [
  'Abbott Street',
  'Aberdeen Street',
  'Adanac Street',
  'Adera Street',
  'Aegean Crescent',
  'Aisne Street',
  'Alamein Avenue',
  'Alberni Street',
  'Alberta Street',
  'Alder Bay Court',
  'Alder Bay Walk',
  'Alder Crossing',
  'Alder Street',
  'Alexander Street',
  'Alexandra Street',
  'Algonquin Mews',
  'Alice Street',
  'Alma Street',
  'Alvin Narod Mews',
  'Amberly Place',
  'Ancaster Crescent',
  'Anderson Street',
  'Anglers Place',
  'Angus Drive',
  'Ann Street',
  'Antwerp Lane',
  'Anzio Drive',
  'Appledale Place',
  'Aqua Drive',
  'Aquarius Mews',
  'Aquitania Place',
  'Arbor Avenue',
  'Arbutus Diversion',
  'Arbutus Street',
  'Archimedes Street',
  'Argyle Drive',
  'Argyle Street',
  'Arlington Street',
  'Ascot Place',
  'Ash Crescent',
  'Ash Street',
  'Ashburn Street',
  'Aspen Avenue',
  'Athletes Way',
  'Athlone Street',
  'Atlantic Street',
  'Atlin Street',
  'Aubrey Place',
  'Austrey Avenue',
  'Avery Avenue',
  'Avison Way',
  'Avondale Avenue',
  'Bach Avenue',
  'Baillie Street',
  'Balaclava Street',
  'Baldwin Street',
  'Balfour Avenue',
  'Balkan Street',
  'Balmoral Street',
  'Balsam Place',
  'Balsam Street',
  'Barclay Street',
  'Barnard Street',
  'Battison Street',
  'Bauer Street',
  'Bayshore Drive',
  'Bayshore Place',
  'Bayswater Street',
  'Beach Avenue',
  'Beach Crescent',
  'Beadnell Court',
  'Beagle Court',
  'Beamish Court',
  'Beatrice Street',
  'Beatty Street',
  'Beatty Walk',
  'Beechwood Street',
  'Begg Street',
  'Bella Vista Street',
  'Bellevue Drive',
  'Belmont Avenue',
  'Bentall Street',
  'Bentley Street',
  'Bering Avenue',
  'Berkeley Street',
  'Beverly Crescent',
  'Bidwell Street',
  'Birch Street',
  'Birch Walk',
  'Blake Street',
  'Blanca Street',
  'Blanche Street',
  'Blaydon Court',
  'Blenheim Street',
  'Blood Alley Square',
  'Blue Jay Crescent',
  'Boathouse Mews',
  'Boatlift Lane',
  'Bobolink Avenue',
  'Bonaccord Drive',
  'Bonnyvale Avenue',
  'Bonsai Street',
  'Borden Street',
  'Borham Crescent',
  'Boundary Road',
  'Boyd Diversion',
  'Braeburn Street',
  'Brahms Avenue',
  'Brakenridge Street',
  'Brandywine Place',
  'Brant Street',
  'Briar Avenue',
  'Bridgeway',
  'Brigadoon Avenue',
  'Brightwood Place',
  'Brock Street',
  'Brooks Street',
  'Broughton Street',
  'Bruce Street',
  'Brule Mews',
  'Brunswick Street',
  'Bucketwheel',
  'Burkholder Drive',
  'Burnaby Street',
  'Burquitlam Drive',
  'Burrard Street',
  'Bursill Street',
  'Buscombe Street',
  'Bute Street',
  'Butler Street',
  'Bywell Court',
  'Camano Street',
  'Cambie Street',
  'Cambridge Street',
  'Cameron Avenue',
  'Camosun Street',
  'Campbell Avenue',
  'Canada Place',
  'Capri Crescent',
  'Captains Cove',
  'Cardero Street',
  'Cariboo Street',
  'Carlisle Street',
  'Carnarvon Street',
  'Carolina Street',
  'Carrall Street',
  'Carrington Street',
  'Cartier Place',
  'Cartier Street',
  'Cartwright Street',
  'Cassiar Street',
  'Caversham Court',
  'Cecil Street',
  'Cedar Cottage Mews',
  'Cedar Crescent',
  'Cedarhurst Street',
  'Celista Drive',
  'Celtic Avenue',
  'Centennial Road',
  'Central Street',
  'Chaldecott Street',
  'Chambers Street',
  'Chambord Place',
  'Champlain Crescent',
  'Champlain Mall',
  'Chandlery Place',
  'Charles Street',
  'Charleson',
  'Chateau Place',
  'Chatham Street',
  'Cheam Place',
  'Cherry Street',
  'Chess Street',
  'Chester Street',
  'Chestnut Street',
  'Cheyenne Avenue',
  'Chickadee Place',
  'Chilco Street',
  'Church Street',
  'Churchill Street',
  'Citadel Parade',
  'City Edge Place',
  'Clancy Loranger Way',
  'Clarendon Street',
  'Clark Drive',
  'Cleadon Court',
  'Clermont Mews',
  'Clive Avenue',
  'Coal Harbour Quay',
  'Cobblestone Avenue',
  'Coleridge Avenue',
  'Colfax Crescent',
  'College Street',
  'Collingwood Place',
  'Collingwood Street',
  'Columbia Street',
  'Commercial Drive',
  'Commercial Street',
  'Commissioner Street',
  'Commodore Road',
  'Comox Street',
  'Connaught Drive',
  'Cook Street',
  'Cooperage Way',
  'Cooperative Way',
  'Coopers Mews',
  'Copeland Avenue',
  'Copley Street',
  'Coquihalla Drive',
  'Coral Reef Place',
  'Cordiale Drive',
  'Cordova Diversion',
  'Cornerstone Street',
  'Cornett Road',
  'Cornish Street',
  'Cornwall Avenue',
  'Corsica Way',
  'Cotton Drive',
  'Cottrell Street',
  'Couillard Mews',
  'Courtenay Street',
  'Cranberry Drive',
  'Cree Street',
  'Creekside Drive',
  'Creelman Avenue',
  'Cresswell Court',
  'Crompton Street',
  'Cromwell Street',
  'Crowe Street',
  'Crowley Drive',
  'Crown Crescent',
  'Crown Place',
  'Crown Street',
  'Culloden Street',
  'Curate Wynd',
  'Cypress Crescent',
  'Cypress Street',
  'Dartmoor Place',
  'Davie Street',
  'Dawson Street',
  'Dease Lane',
  'Deering Island Place',
  'Denman Street',
  'Devonshire Crescent',
  'Dieppe Drive',
  'Dieppe Lane',
  'Dieppe Place',
  'Dinmont Avenue',
  'Discovery Street',
  'Dogwood Avenue',
  'Doman Street',
  'Doncaster Way',
  'Douglas Crescent',
  'Drake Street',
  'Drummond Drive',
  'Duchess Street',
  'Duff Street',
  'Duke Street',
  'Dumfries Street',
  'Dunbar Diversion',
  'Dunbar Street',
  'Dundas Street',
  'Dundee Street',
  'Dunelm Avenue',
  'Dunkirk Street',
  'Dunlevy Avenue',
  'Dunsmuir Street',
  'Duranleau Street',
  'Durward Avenue',
  'Earl Finning Place',
  'Earles Street',
  'East 10th Avenue',
  'East 11th Avenue',
  'East 12th Avenue',
  'East 13th Avenue',
  'East 14th Avenue',
  'East 15th Avenue',
  'East 16th Avenue',
  'East 17th Avenue',
  'East 18th Avenue',
  'East 19th Avenue',
  'East 1st Avenue',
  'East 20th Avenue',
  'East 21st Avenue',
  'East 22nd Avenue',
  'East 23rd Avenue',
  'East 24th Avenue',
  'East 25th Avenue',
  'East 26th Avenue',
  'East 27th Avenue',
  'East 28th Avenue',
  'East 29th Avenue',
  'East 2nd Avenue',
  'East 30th Avenue',
  'East 31st Avenue',
  'East 32nd Avenue',
  'East 33rd Avenue',
  'East 34th Avenue',
  'East 35th Avenue',
  'East 36th Avenue',
  'East 37th Avenue',
  'East 38th Avenue',
  'East 39th Avenue',
  'East 3rd Avenue',
  'East 40th Avenue',
  'East 41st Avenue',
  'East 42nd Avenue',
  'East 43rd Avenue',
  'East 44th Avenue',
  'East 45th Avenue',
  'East 46th Avenue',
  'East 47th Avenue',
  'East 48th Avenue',
  'East 49th Avenue',
  'East 4th Avenue',
  'East 50th Avenue',
  'East 51st Avenue',
  'East 52nd Avenue',
  'East 53rd Avenue',
  'East 54th Avenue',
  'East 55th Avenue',
  'East 56th Avenue',
  'East 57th Avenue',
  'East 58th Avenue',
  'East 59th Avenue',
  'East 5th Avenue',
  'East 60th Avenue',
  'East 61st Avenue',
  'East 62nd Avenue',
  'East 63rd Avenue',
  'East 64th Avenue',
  'East 65th Avenue',
  'East 66th Avenue',
  'East 67th Avenue',
  'East 69th Avenue',
  'East 6th Avenue',
  'East 7th Avenue',
  'East 8th Avenue',
  'East Boulevard',
  'East Broadway',
  'East Cordova Street',
  'East Georgia Street',
  'East Hastings Street',
  'East Kent Avenue North',
  'East Kent Avenue South',
  'East King Edward Avenue',
  'East Pender Street',
  'East Waterfront Road',
  'East Woodstock Avenue',
  'Echo Place',
  'Eddington Drive',
  'Edgar Crescent',
  'Edinburgh Street',
  'Elgin Street',
  'Elizabeth Street',
  'Elk Valley Place',
  'Elliott Street',
  'Elm Street',
  'Elmhurst Drive',
  'Eton Street',
  'Euclid Avenue',
  'Evans Avenue',
  'Eveleigh Street',
  'Explorers Walk',
  'Expo Boulevard',
  'Fairchild Road',
  'Fairmont Street',
  'Falaise Avenue',
  'Falaise Lane',
  'Falaise Place',
  'Fannin Avenue',
  'Fellowes Street',
  'Ferndale Street',
  'Ferry Row',
  'Fieldstone Avenue',
  'Finch Place',
  'Findlay Street',
  'Fir Street',
  'Flagstaff Place',
  'Fleming Street',
  'Foley Street',
  'Foreshore Walk',
  'Forge Walk',
  'Foster Avenue',
  'Foundry Quay',
  'Fountain Way',
  'Foxhound Mews',
  'Frances Street',
  'Franklin Street',
  'Fraser Street',
  'Fraserview Drive',
  'Frederick Street',
  'Fremlin Street',
  'French Street',
  'Frontenac Street',
  'Galiano Road',
  'Galt Street',
  'Gaolers Mews',
  'Garden Drive',
  'Garibaldi Drive',
  'Gaston Street',
  'Gatineau Place',
  'George Street',
  'Ghent Lane',
  'Gilford Street',
  'Gillnet Place',
  'Gladstone Street',
  'Glen Drive',
  'Glendale Street',
  'Glendalough Place',
  'Glengyle Street',
  'Golden Link Crescent',
  'Gore Avenue',
  'Gothard Street',
  'Grandview Highway',
  'Grandview Highway North',
  'Grandview Highway South',
  'Grant Street',
  'Granville Street',
  'Graveley Street',
  'Great Northern Way',
  'Greenchain',
  'Greensboro Place',
  'Greer Avenue',
  'Griffiths Way',
  'Grouse Walk',
  'Guelph Street',
  'Gwillim Crescent',
  'Haggart Street',
  'Haida Drive',
  'Haig Street',
  'Halss Crescent',
  'Hamilton Street',
  'Handel Avenue',
  'Haro Street',
  'Harold Street',
  'Harriet Street',
  'Harrison Drive',
  'Harwood Street',
  'Hawks Avenue',
  'Hazelton Street',
  'Heather Street',
  'Heatley Avenue',
  'Hebb Avenue',
  'Hecate Place',
  'Helmcken Street',
  'Hemlock Court',
  'Hemlock Street',
  'Hennepin Avenue',
  'Henry Street',
  'Hermon Drive',
  'Heron Place',
  'Highbury Street',
  'Highgate Street',
  'Highway 1a',
  'Highway 99',
  'Holland Street',
  'Hollyhock Lane',
  'Homer Mews',
  'Homer Street',
  'Horley Street',
  'Hornby Street',
  'Hosmer Avenue',
  'Howe Street',
  'Hoy Street',
  'Hoylake Avenue',
  'Hudson Street',
  'Hull Street',
  'Hum-lu-sum Drive',
  'Humm Street',
  'Hummingbird Place',
  'Huron Mews',
  'Hurst Avenue',
  'Hurst Place',
  'Hyack Drive',
  'Imperial Road',
  'Industrial Avenue',
  'Inverness Street',
  'Ironwork Passage',
  'Isabel Place',
  'Island Avenue',
  'Island Park Walk',
  'Ivanhoe Street',
  'Jackson Avenue',
  'Jadetree Court',
  'James Street',
  'James Walk',
  'Jasper Crescent',
  'Jellicoe Street',
  'Jericho Circle',
  'Jericho Drive',
  'Jervis Mews',
  'Jervis Street',
  'John Street',
  'Johnston Street',
  'Joyce Street',
  'Kamloops Street',
  'Kanata',
  'Kaslo Street',
  'Ke Kait Place',
  'Keefer Place',
  'Keefer Street',
  'Keith Drive',
  'Kelowna Street',
  'Kempton Place',
  'Kerr Street',
  'Kersland Drive',
  'Kevin Place',
  'Keystone Street',
  'Kielder Court',
  'Killarney Street',
  'Killdeer Place',
  'King Edward Avenue',
  'Kinghorne Mews',
  'Kings Avenue',
  'Kings Lynn Street',
  'Kingsway',
  'Kinross Street',
  'Kirkland Street',
  'Kitchener Street',
  'Kitsilano Diversion',
  'Knight Street',
  'Kokanee Place',
  'Kootenay Street',
  'Kullahun Drive',
  'La Salle Street',
  'Laburnum Street',
  'Lagoon Drive',
  'Lakewood Drive',
  'Lakewood Street',
  'Lameys Mill Road',
  'Lanark Street',
  'Lancaster Place',
  'Lancaster Street',
  'Langara Avenue',
  'Langara Golf Course Access',
  'Langford Avenue',
  'Larch Street',
  'Latta Street',
  'Laurel Street',
  'Laurier Avenue',
  'Laval Place',
  'Le Roi Street',
  'Leaside Street',
  'Leg-in-boot Square',
  'Liard Place',
  'Lighthouse Way',
  'Lillooet Street',
  'Lily Street',
  'Lime Street',
  'Limewood Place',
  'Lincoln Street',
  'Linden Road',
  'Little Street',
  'Locarno Crescent',
  'Logan Street',
  'Long Life Place',
  'Lord Street',
  'Lorne Street',
  'Lost Lagoon Road',
  'Lougheed Highway',
  'Lynbrook Drive',
  'Lynmoor Place',
  'Macdonald Street',
  'Mackenzie Place',
  'Mackenzie Street',
  'Mackie Street',
  'Maclure Mews',
  'Macmillan Drive',
  'Maddams Street',
  'Magnolia Street',
  'Main Street',
  'Mainland Street',
  'Mali Avenue',
  'Mali Lane',
  'Malkin Avenue',
  'Malta Avenue',
  'Malta Place',
  'Manitoba Street',
  'Mannering Avenue',
  'Manor Street',
  'Manson Street',
  'Maple Crescent',
  'Maple Street',
  'Maquinna Drive',
  'Marchwood Place',
  'Marguerite Street',
  'Marinaside Crescent',
  'Marine Crescent',
  'Marine Way',
  'Mariner Walk',
  'Maritime Mews',
  'Market Hill',
  'Marlin Quay',
  'Marmion Avenue',
  'Marpole Avenue',
  'Marquette Crescent',
  'Marshall Street',
  'Marstrand Avenue',
  'Mast Tower Road',
  'Matapan Crescent',
  'Matheson Crescent',
  'Matthews Avenue',
  'Maxwell Street',
  'Mayfair Avenue',
  'Mcbain Avenue',
  'Mccleery Street',
  'Mcgeer Street',
  'Mcgill Street',
  'Mcguigan Avenue',
  'Mchardy Street',
  'Mcintyre Drive',
  'Mckinnon Street',
  'Mclean Drive',
  'Mcmullen Avenue',
  'Mcnicoll Avenue',
  'Mcrae Avenue',
  'Mcspadden Avenue',
  'Meadowland Place',
  'Meares Avenue',
  'Melbourne Street',
  'Melville Street',
  'Menchions Mews',
  'Midlothian Avenue',
  'Millbank',
  'Miller Drive',
  'Miller Street',
  'Millstone Street',
  'Milltown Place',
  'Millyard',
  'Milross Avenue',
  'Milton Street',
  'Minto Crescent',
  'Moberly Road',
  'Mohawk Street',
  'Monmouth Avenue',
  'Mons Drive',
  'Mont Royal Square',
  'Montcalm Street',
  'Montgomery Street',
  'Morningstar Crescent',
  'Morton Avenue',
  'Moscrop Street',
  'Moss Street',
  'Muirfield Drive',
  'Munroe Crescent',
  'Musqueam Avenue',
  'Musqueam Close',
  'Musqueam Drive',
  'Nairn Avenue',
  'Nanaimo Street',
  'Nanton Avenue',
  'Napier Street',
  'Naples Way',
  'Narvaez Drive',
  'Nassau Drive',
  'Natal Street',
  'National Avenue',
  'Nautilus Close',
  'Neal Street',
  'Nelson Street',
  'New Brighton Road',
  'Newcastle Court',
  'Newport Avenue',
  'Nicola Street',
  'Nigel Avenue',
  'None',
  'Nootka Street',
  'Normandy Drive',
  'Normandy Road',
  'Norquay Street',
  'North Boundary Road',
  'North Cassiar Street',
  'North Commercial Drive',
  'North Garden Drive',
  'North Kamloops Street',
  'North Kaslo Street',
  'North Kootenay Street',
  'North Lagoon Drive',
  'North Lakewood Drive',
  'North Nanaimo Street',
  'North Penticton Street',
  'North Renfrew Street',
  'North Salsbury Drive',
  'North Skeena Street',
  'North Slocan Street',
  'North Templeton Drive',
  'North Victoria Drive',
  'North Windermere Street',
  'Northern Street',
  'Northumberland Avenue',
  'Northwest Marine Drive',
  'Oak Street',
  'Oben Street',
  'Odlum Drive',
  'Ogden Avenue',
  'Old Bridge Court',
  'Old Bridge Street',
  'Old Bridge Walk',
  'Oliver Crescent',
  'Olympic Street',
  'Ontario Place',
  'Ontario Street',
  'Ormidale Street',
  'Ortona Crescent',
  'Ortona Lane',
  'Osler Street',
  'Oxford Street',
  'Pacific Boulevard',
  'Pacific Street',
  'Pandora Street',
  'Park Drive',
  'Park Lane',
  'Parker Street',
  'Parkway Drive',
  'Paton Street',
  'Payne Street',
  'Pendrell Street',
  'Pennyfarthing Drive',
  'Penticton Street',
  'Perrot Mews',
  'Perry Street',
  'Petersham Avenue',
  'Peveril Avenue',
  'Picton Street',
  'Pine Crescent',
  'Pine Street',
  'Pinehurst Drive',
  'Pinnacle Court',
  'Pipeline Road',
  'Point Grey Road',
  'Poplar Street',
  'Porter Street',
  'Portside Court',
  'Portside Drive',
  'Powell Street',
  'Prescott Street',
  'Prestwick Drive',
  'Price Street',
  'Prince Albert Street',
  'Prince Edward Street',
  'Princess Avenue',
  'Prior Street',
  'Promenade Mews',
  'Prospect Point Park Lane',
  'Puget Drive',
  'Quadra Street',
  'Qualicum Drive',
  'Quatsino Drive',
  'Quayside Court',
  'Quebec Place',
  'Quebec Street',
  'Queens Avenue',
  'Quesnel Drive',
  'Quilchena Crescent',
  'Quilchena Place',
  'Radisson Street',
  'Rae Avenue',
  'Railspur Alley',
  'Railway Street',
  'Rainier Place',
  'Raintree Court',
  'Raleigh Street',
  'Rand Avenue',
  'Ravine Street',
  'Raymur Avenue',
  'Rebekah Drive',
  'Redbud Lane',
  'Regiment Square',
  'Reid Street',
  'Renfrew Street',
  'Rhodes Street',
  'Richards Street',
  'Richelieu Avenue',
  'Riel Place',
  'Ringwood Avenue',
  'Riverwood Way',
  'Robson Street',
  'Rockpoole Drive',
  'Rogers Street',
  'Rolston Crescent',
  'Rose Garden Lane',
  'Rose Street',
  'Rosebury Lane',
  'Rosebury Walk',
  'Rosedale Drive',
  'Rosemont Drive',
  'Ross Street',
  'Rossland Street',
  'Roundhouse Mews',
  'Roxburgh Crescent',
  'Ruby Street',
  'Rumble Avenue',
  'Rupert Street',
  'Saddle Lane',
  'Saint Julien Mews',
  'Salal Drive',
  'Salerno Street',
  'Salish Drive',
  'Salsbury Drive',
  'Salt Street',
  'Sasamat Place',
  'Sasamat Street',
  'Sawcut',
  'Sawyers Lane',
  'Scales Place',
  'Scantlings',
  'Scarboro Avenue',
  'School Avenue',
  'School Green',
  'Scotia Street',
  'Seabreeze Walk',
  'Seaforth Drive',
  'Seagull Place',
  'Selkirk Street',
  'Semana Crescent',
  'Semlin Drive',
  'Senlac Street',
  'Sennok Crescent',
  'Seymour Mews',
  'Seymour Street',
  'Shanghai Alley',
  'Shaughnessy Street',
  'Shawnee Place',
  'Sherbrooke Street',
  'Shorepine Walk',
  'Si-lu Drive',
  'Sidney Street',
  'Sienum Place',
  'Simpson Avenue',
  'Sitka Square',
  'Skana Drive',
  'Skeena Street',
  'Skipper Place',
  'Slocan Street',
  'Smithe Mews',
  'Smithe Street',
  'Somerset Crescent',
  'Somerville Street',
  'Sophia Street',
  'Sotao Avenue',
  'South Shore Crescent',
  'Southeast Marine Drive',
  'Southern Street',
  'Southlands Place',
  'Southside Drive',
  'Southwest Marine Drive',
  'Sparbrook Crescent',
  'Sparrow Place',
  'Spencer Street',
  'Spinnaker Place',
  'Springtree Drive',
  'Spruce Street',
  'Spyglass Place',
  'St Catherines Street',
  'St Clair Place',
  'St George Street',
  'St Lawrence Street',
  'St Margarets Street',
  'St Regis Lane',
  'Stainsbury Avenue',
  'Stamford Street',
  'Stamps Landing',
  'Stanley Park Causeway',
  'Stanley Park Drive',
  'Starboard Square',
  'Station Street',
  'Staulo Crescent',
  'Stautlo Avenue',
  'Stephens Street',
  'Stewart Street',
  'Stikine Place',
  'Stirling Street',
  'Strathmore Mews',
  'Strauss Drive',
  'Suzhou Alley',
  'Swallow Place',
  'Swansacre',
  'Talisman Avenue',
  'Talon Square',
  'Tamarind Drive',
  'Tamath Crescent',
  'Tanner Street',
  'Tantalus Lane',
  'Taunton Street',
  'Taylor Street',
  'Teakwood Place',
  'Tecumseh Avenue',
  'Templeton Drive',
  'Terminal Avenue',
  'Terra Vita Place',
  'Terry Fox Way',
  'The Castings',
  'The Crescent',
  'Thellaiwhaltun Avenue',
  'Thornhill Drive',
  'Thornton Street',
  'Three Cedars Drive',
  'Thrush Place',
  'Thurlow Street',
  'Tidewater Place',
  'Tiffany Ridge',
  'Tisdall Street',
  'Toba Place',
  'Todd Street',
  'Toderick Street',
  'Tolbooth',
  'Tolmie Street',
  'Townley Street',
  'Trafalgar Street',
  'Trans-canada Highway',
  'Trimble Street',
  'Trinity Street',
  'Triumph Street',
  'Trolley Place',
  'Trounce Alley',
  'Trutch Street',
  'Tuam Place',
  'Tuaq Drive',
  'Tugboat Place',
  'Tupper Street',
  'Turnberry Crescent',
  'Turner Street',
  'Tyne Street',
  'Tytahun Crescent',
  'Union Street',
  'Upland Drive',
  'Valdez Road',
  'Valley Drive',
  'Vanness Avenue',
  'Venables Street',
  'Vernon Drive',
  'Victoria Diversion',
  'Victoria Drive',
  'Vimy Crescent',
  'Vine Street',
  'Virtual Way',
  'Vivaldi Place',
  'Vivian Drive',
  'Vivian Street',
  'Walden Street',
  'Wales Street',
  'Walker Street',
  'Wall Street',
  'Wallace Crescent',
  'Wallace Place',
  'Wallace Street',
  'Walnut Street',
  'Walter Hardwick Avenue',
  'Waneta Place',
  'Ward Street',
  'Water Street',
  'Waterford Drive',
  'Waterfront Road',
  'Waterleigh Drive',
  'Waterloo Street',
  'Waterside Avenue',
  'Watson Street',
  'Waverley Avenue',
  'Weaver Court',
  'Webber Avenue',
  'Wellington Avenue',
  'Welwyn Street',
  'Wenonah Street',
  'Werks Drive',
  'Wessex Street',
  'West 10th Avenue',
  'West 11th Avenue',
  'West 12th Avenue',
  'West 13th Avenue',
  'West 14th Avenue',
  'West 15th Avenue',
  'West 16th Avenue',
  'West 17th Avenue',
  'West 18th Avenue',
  'West 19th Avenue',
  'West 1st Avenue',
  'West 20th Avenue',
  'West 21st Avenue',
  'West 22nd Avenue',
  'West 23rd Avenue',
  'West 24th Avenue',
  'West 26th Avenue',
  'West 27th Avenue',
  'West 28th Avenue',
  'West 29th Avenue',
  'West 2nd Avenue',
  'West 30th Avenue',
  'West 31st Avenue',
  'West 32nd Avenue',
  'West 33rd Avenue',
  'West 34th Avenue',
  'West 35th Avenue',
  'West 36th Avenue',
  'West 37th Avenue',
  'West 38th Avenue',
  'West 39th Avenue',
  'West 3rd Avenue',
  'West 40th Avenue',
  'West 41st Avenue',
  'West 42nd Avenue',
  'West 43rd Avenue',
  'West 44th Avenue',
  'West 45th Avenue',
  'West 46th Avenue',
  'West 47th Avenue',
  'West 48th Avenue',
  'West 49th Avenue',
  'West 4th Avenue',
  'West 50th Avenue',
  'West 51st Avenue',
  'West 52nd Avenue',
  'West 53rd Avenue',
  'West 54th Avenue',
  'West 55th Avenue',
  'West 56th Avenue',
  'West 57th Avenue',
  'West 58th Avenue',
  'West 59th Avenue',
  'West 5th Avenue',
  'West 60th Avenue',
  'West 61st Avenue',
  'West 62nd Avenue',
  'West 63rd Avenue',
  'West 64th Avenue',
  'West 65th Avenue',
  'West 66th Avenue',
  'West 67th Avenue',
  'West 68th Avenue',
  'West 69th Avenue',
  'West 6th Avenue',
  'West 70th Avenue',
  'West 71st Avenue',
  'West 72nd Avenue',
  'West 73rd Avenue',
  'West 74th Avenue',
  'West 75th Avenue',
  'West 76th Avenue',
  'West 77th Avenue',
  'West 7th Avenue',
  'West 8th Avenue',
  'West 9th Avenue',
  'West Boulevard',
  'West Broadway',
  'West Cordova Street',
  'West Georgia Street',
  'West Hastings Street',
  'West Kent Avenue North',
  'West Kent Avenue South',
  'West King Edward Avenue',
  'West Pender Street',
  'West Point Place',
  'West Waterfront Road',
  'West Woodstock Avenue',
  'Westbank Place',
  'Westbury Lane',
  'Westbury Walk',
  'Western Street',
  'Wethersfield Drive',
  'Weymoor Place',
  'Wheelhouse Square',
  'Whitespray Drive',
  'Whithorn Court',
  'Whitney Place',
  'Whyte Avenue',
  'William Street',
  'Willow Street',
  'Wiltshire Street',
  'Windermere Street',
  'Windsor Street',
  'Winlaw Place',
  'Wolfe Avenue',
  'Woodland Drive',
  'Worthington Drive',
  'Worthington Place',
  'Wylie Street',
  'Yale Street',
  'Yardley Avenue',
  'Yew Street',
  'Yewbrook Place',
  'York Avenue',
  'Yuculta Crescent',
  'Yukon Street',
];

export default class Initializer {
  private readonly geographySvc: GeographyService;
  private readonly playerSvc: PlayerService;

  constructor(geographySvc: GeographyService, playerSvc: PlayerService) {
    this.geographySvc = geographySvc;
    this.playerSvc = playerSvc;
  }

  async init() {
    const neighbourhoodIds = [];
    for (const name of NEIGHBOURHOOD_NAMES) {
      const id = await this.geographySvc.createNeighbourhood(name);
      neighbourhoodIds.push(id);
    }

    const streetIds: string[] = [];
    for (const name of STREET_NAMES) {
      const id = await this.geographySvc.createStreet(name);
      streetIds.push(id);
    }

    function randomStreetId(): string {
      const streetIdx = Math.floor(Math.random() * streetIds.length);
      return streetIds[streetIdx];
    }

    // Create a couple blocks in every neighbourhood
    for (const neighbourhoodId of neighbourhoodIds) {
      const streetId = randomStreetId();
      // TODO: Ensure the block is free before trying to use it.
      await this.geographySvc.assignBlockToNeighbourhood(streetId, neighbourhoodId, 1, 100);
    }

    // Create a demo player
    await this.playerSvc.createPlayer('Player 1');
  }
}
